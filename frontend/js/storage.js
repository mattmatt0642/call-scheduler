...
const DEFAULT_STATE = {
 ...
 blackouts: {}, // "YYYY-MM": { doctorId: [{date, period, startTime?, endTime?}, ...] }
  schedules: {},     // "YYYY-MM": ScheduleResult
  totals: {},        // doctorId -> cumulative balance dict
};
...
function updateTotalsFromSchedule(monthResult) {
  // After a successful generate, recompute STATE.totals from all saved schedules.
  // Do NOT just add the new month — recompute from scratch to avoid drift.
  const totals = {};
...
// Helper: ensure a doctor exists with all required fields
...
// Initialize on load — localStorage first; server sync happens after auth
