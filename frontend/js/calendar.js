...
function buildICSEvent(slot, docName, officeName) {
  ...
  const description = `${times.label} shift at ${officeName || 'Hospital'} — Generated by ${THEME_CONFIG.name}`;
...
function generateICS(year, month, scheduleData) {
  ...
  const prodId = `-//${THEME_CONFIG.name}//Medical Schedule//EN`;
...
function openGoogleCalendar(year, month, scheduleData) {
  ...
  const details = encodeURIComponent(`${times.label} shift at ${officeName} — Generated by ${THEME_CONFIG.name}`);
